    ...
    for raw_inst in data:
        ...
        if len(raw_entities) > 0:
            ...
            for eid, e in enumerate(raw_entities):
                ...
                try:
                    ...
                    entities.append({
                        'label': CHEMPROT_ENTITY_TYPES.index(e['type']),
        ...
        if len(raw_relations) > 0:
            for relation in raw_relations:
                ...
                label = relation['type']
                try:
                    relation_label = CHEMPROT_RELATION_TYPES.index(label)
                    relations.append({
                        'participants': [p1, p2],
...
def load_chemprot_dataset(base_path, tokenizer, split_nb):
    # Determine absolute file paths
    assert(split_nb in list(range(10)))
    train_fp = join(base_path, 'chemprot_split_{}_train.json'.format(split_nb))
    test_fp = join(base_path, 'chemprot_split_{}_test.json'.format(split_nb))
    types_fp = join(base_path, 'chemprot_types.json')
